...
import { setName, setEmail, setPhone, setMessage, setIsSendedMessage } from '../../store/contactSlice'
...
const ContactForm = () => {
    ...
    const sendMessage = async () => {
        await emailjs.send(serviceKey, templateKey, contactForm)
        ...
        <div className='contact-form'>
            <div className='contact-form-fields'>
                ...
                <TextBox
                    ...
                    onChange={onChangeEmail}></TextBox>
                <TextBox
                    pattern={'Your phone number: *'}
                    regex={/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/}
                    inputHelp={'Invalid phone number'}
                    onChange={onChangePhone}></TextBox>
                <TextBox
                    pattern={'Your message: *'}
                    inputHelp={'I`m waiting for 0/1 letters'}
                    onChange={onChangeMessage}></TextBox>
            </div>

                <button className='contact-form__submit' type='button' onClick={() => { onHadleSubmit() }}>Submit</button>
        </div>
    )
}
